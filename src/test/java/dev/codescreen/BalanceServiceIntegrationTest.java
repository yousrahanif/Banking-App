...
class BalanceServiceIntegrationTest {
	...
	void testProcessLoadIntegration() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.put("/loads").content(asJsonString(request))
	...
	void testProcessAuthorizationIntegration() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.put("/loads").content(asJsonString(loadRequest))
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andExpect(MockMvcResultMatchers.status().isOk());
		...
		mockMvc.perform(MockMvcRequestBuilders.put("/authorizations").content(asJsonString(authRequest))
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(MockMvcResultMatchers.content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
				.andExpect(MockMvcResultMatchers.content().string("20.0")); // Expect the updated balance after
																			// authorization
	}
